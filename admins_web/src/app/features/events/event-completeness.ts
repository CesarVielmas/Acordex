...
// Los tipos del checklist y el cierre del reporte viven en
// `core/models/completeness.models`: son idénticos para un evento con boletaje y
// para una firma de prensa, y lo único propio de cada uno es la lista de puntos.
// Se reexportan aquí porque medio módulo de Eventos los importa desde este
// archivo desde antes de que Prensa existiera.
export type {
  CompletenessGroup,
  CompletenessItem,
  PendingOwnerGroup,
...
export function eventCompleteness(e: EventItem): CompletenessReport {
  ...
  const rawItems: CompletenessDraft[] = [
  ...
  return buildCompletenessReport(rawItems, organizer);
}
