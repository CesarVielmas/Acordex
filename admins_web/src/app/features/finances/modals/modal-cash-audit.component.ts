      ...
      <div class="w-full max-w-xl rounded-3xl bg-[#1A1A1A] border border-white/10 shadow-2xl p-6 space-y-5">
        ...
        <div class="flex items-center justify-between border-b border-white/10 pb-3">
          <div class="flex items-center gap-2.5">
            ...
            <div>
              <h3 class="text-base font-black text-on-surface font-['Epilogue']">Acta de Arqueo y Conciliación de Tesorería</h3>
              <p class="text-[11px] text-outline font-['Epilogue']">Certificación de saldos en cuentas bancarias receptoras y caja chica</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <div class="p-6 rounded-2xl bg-[#141414] border border-white/5 space-y-4 font-mono text-xs shadow-inner">
          ...
          <div class="flex justify-between items-center border-b border-white/10 pb-3">
            <div>
              <span class="font-sans font-black text-sm text-on-surface block font-['Epilogue']">Acordex Records S.A. de C.V.</span>
              <span class="text-[10px] text-outline font-sans">Dirección Corporativa de Finanzas & Tesorería</span>
            </div>
          ...
          <div class="space-y-2 pt-1">
            @for (acc of accounts(); track acc.id) {
              <div class="flex items-center justify-between p-2.5 rounded-xl bg-[#1A1A1A] border border-white/5">
                <div>
                  <span class="font-sans font-bold text-on-surface block font-['Epilogue']">{{ acc.name }}</span>
          ...
          <div class="flex items-center justify-between p-4 rounded-xl bg-primary/10 border border-primary/30 text-sm">
            <span class="font-sans font-black text-on-surface font-['Epilogue']">Total Liquidez en Cuentas:</span>
            <span class="text-base font-black text-emerald-300 font-mono">{{ money(totalCash()) }}</span>
          </div>

          <!-- Sello Digital de Auditoría -->
          <div class="p-3 rounded-xl bg-black/60 border border-white/5 text-[10px] text-outline space-y-1">
            <div class="flex justify-between">
              <span>Sello Digital de Validación:</span>
              <span class="font-bold text-on-surface">ACX-FIN-SEC-{{ timestamp() }}</span>
            </div>
            <div class="flex justify-between">
              <span>Auditor Responsable:</span>
              <span class="text-on-surface font-sans font-semibold">Lic. Claudia Morales (Encargada Global)</span>
            </div>
          </div>
        ...
        <div class="flex items-center justify-end gap-2 pt-2 border-t border-white/10">
          ...
          <button
            type="button"
            (click)="printAudit()"
            class="px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary to-primary-hover text-on-primary text-xs font-black shadow-lg shadow-primary/20 transition-all flex items-center gap-1.5 cursor-pointer font-['Epilogue']"
          >
