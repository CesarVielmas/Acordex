...
@Component({
  ...
  template: `
    ...
    >
      ...
      <!-- Navigation Links Container (Clean Invisible Scrollbar & Categorized Layout) -->
      <div 
        class="p-2.5 sm:p-3 space-y-3 flex-1 overflow-y-auto scrollbar-none [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden transition-all"
      >
        
        <!-- Role Status Indicator Banner (Visible when expanded) -->
        <div 
          *ngIf="!layoutState.isSidebarCollapsed()"
          class="p-3 rounded-2xl bg-gradient-to-br from-[#1E1E1E] to-[#121212] border border-primary/20 relative overflow-hidden shadow-lg transition-all duration-300 mb-2"
        >
          <span class="text-[9.5px] font-extrabold uppercase tracking-widest text-outline block mb-0.5 font-['Epilogue']">Sesión Activa</span>
        ...
        <nav class="space-y-4">
          @for (cat of filteredCategories(); track cat.title) {
            <div class="space-y-1">
              ...
              <!-- Category Header Label (Expanded) -->
              <div 
                *ngIf="!layoutState.isSidebarCollapsed()" 
                class="px-3 pt-1 pb-0.5 text-[9px] font-black uppercase tracking-widest text-outline/60 flex items-center gap-2 font-['Epilogue']"
              >
                <span>{{ cat.title }}</span>
                <span class="h-px bg-white/10 flex-1"></span>
              </div>

              <!-- Category Divider (Collapsed) -->
              <div 
                *ngIf="layoutState.isSidebarCollapsed()" 
                class="w-6 h-px bg-white/10 mx-auto my-2"
              ></div>
              ...
              @for (item of cat.items; track item.route) {
                <a 
                  ...
                  class="flex items-center py-2 rounded-xl text-xs font-medium text-on-surface-variant hover:text-on-surface hover:bg-white/[0.04] transition-all duration-200 group relative"
                  [ngClass]="layoutState.isSidebarCollapsed() ? 'justify-center px-0' : 'justify-between px-3'"
                  [title]="layoutState.isSidebarCollapsed() ? item.label : ''"
                >
                  <div class="flex items-center gap-2.5 min-w-0">
                    <span class="material-symbols-outlined text-lg group-hover:scale-110 transition-transform duration-200 group-hover:text-primary shrink-0">
                      {{ item.icon }}
                    </span>
                    <span 
                      *ngIf="!layoutState.isSidebarCollapsed()" 
                      class="font-['Epilogue'] tracking-wide truncate text-[11.5px]"
                    >
                      {{ item.label }}
                    </span>
                  </div>
                  ...
                  <span 
                    *ngIf="!layoutState.isSidebarCollapsed() && item.badge" 
                    class="text-[8.5px] font-bold px-1.5 py-0.2 rounded-full bg-primary/20 text-primary border border-primary/30 shrink-0"
                  >
                    {{ item.badge }}
                  </span>
      ...
      <div 
        class="p-3 border-t border-white/10 bg-[#111111] transition-all shrink-0"
        [ngClass]="layoutState.isSidebarCollapsed() ? 'text-center' : 'px-4'"
      >
        <div *ngIf="!layoutState.isSidebarCollapsed()" class="flex items-center justify-between text-[10.5px] text-outline font-medium">
          <span class="font-['Epilogue'] tracking-wider">Acordex Admin</span>
          <span class="text-[9.5px] font-extrabold px-2 py-0.5 rounded bg-primary/10 text-primary border border-primary/20">
            v2.4
          </span>
        </div>
