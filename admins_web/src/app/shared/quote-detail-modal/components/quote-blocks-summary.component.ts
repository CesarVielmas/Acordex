              ...
              <div class="p-3.5 rounded-xl bg-surface-container/90 border border-cyan-500/40 flex flex-col justify-between text-xs space-y-2.5 shadow-md hover:border-cyan-400 hover:-translate-y-0.5 transition-all">
                ...
                <div class="space-y-1.5 font-mono">
                  <div class="flex justify-between items-baseline">
                    ...
                    <strong class="text-emerald-400 font-black text-sm text-shadow-sm">&#36;{{ milestoneAmount(m) | number:'1.0-0' }} MXN</strong>
                  </div>
...
export class QuoteBlocksSummaryComponent {
  ...
  milestoneAmount(m: PaymentMilestone): number {
    const baseTotal = this.quote?.totalAmount || 0;
    if (m.type === 'percentage') {
      return (baseTotal * m.percentageOrAmount) / 100;
    }
